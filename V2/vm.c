...
// auteur Karim Baïna, ENSIAS, Décembre 2010, mise à jour Décembre 2011
  ...
  *next_label_name = NULL;
  ...
  switch(pi.codop){
    case DATA:
		...
		varvalueType nv;
		nv.name = (char*) malloc(sizeof(char) * strlen(pi.param.nv.name)+1);
		strcpy(nv.name, pi.param.nv.name);
		nv.valinit = pi.param.nv.value;
